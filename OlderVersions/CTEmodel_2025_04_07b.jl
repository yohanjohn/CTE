...
function W(receiver::Float64, sender::Float64)
    ...
    #M[infra,supra] = amp * max(sender - receiver,0)
    #M[supra,infra] = amp * max(receiver - sender,0)
    
    M[infra,infra] = amp * (1- abs(sender - receiver))
    M[supra,supra] = amp * (1- abs(receiver - sender))
